...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, x, cross):
        batch, n_vars, _, d_model = x.shape
        x = x.reshape(batch*n_vars, n_vars, d_model)
        ...
        x_cross , _ = self.cross_attention(cross, x_attn, x_attn)
        x_cross = self.dropout(self.norm2(x_cross)) + cross

        x_ff = self.MLP1(x_cross)
        x_ff = self.norm3(x_ff) + x_cross

        x_cross = x_cross.reshape(batch, n_vars, -1, d_model)
