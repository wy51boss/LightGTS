...
class LightGTS(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, target_patch_len: int,num_patch:int, mask_mode:str = 'patch',mask_nums:int = 3,
        ...
        self.patch_size = [24, 48, 96, 192, 288]
        ...
        if head_type == "pretrain":
            self.head = PretrainHead(d_model, self.target_patch_len, head_dropout) # custom head passed as a partial func with all its kwargs
    ...
    def forward(self, z):                             
        ...
        z = z.permute(0, 2, 1, 3) # [bs*mask_nums, n_vars, num_patch, patch_len]
        z = z.reshape(z.shape[0], z.shape[1], -1)
        

        patch_size = random.choice(self.patch_size)
        z = z.reshape(z.shape[0], z.shape[1], -1, patch_size).permute(0, 2, 1, 3)
        self.out_patch_num = math.ceil(self.target_dim / patch_size)
        self.num_patch = z.shape[1]


        # tokenizer
        cls_tokens = self.cls_embedding.expand(bs*(mask_nums + 1), n_vars, -1, -1)
        embedding = nn.Linear(patch_size, self.d_model)
        embedding.weight.data = resample_patchemb(old=self.embedding.weight.data, new_patch_len=patch_size)
        embedding.bias.data = self.embedding.bias.data
        z = embedding(z).permute(0,2,1,3) # [bs*(1+mask_nums) x n_vars x num_patch x d_model]
        ...
        if self.mask_mode == "multi" or self.mask_mode == "freq_multi":
            z_masked = z_masked.reshape(self.mask_nums, -1, n_vars, self.num_patch + 1, self.d_model,).mean(0).transpose(2,3)     # z_masked: [bs x nvars x d_model x num_patch]    

        # decoder_prediction
        z_predict = self.decoder_predict(bs, n_vars, z_original)
        z_predict = self.head(z_predict, patch_size)  
        ...
        # recontruction
        z_reconstruct = self.head(z_masked[:,:,:,1:], patch_size)
        z_reconstruct = z_reconstruct.permute(0, 2, 1, 3).reshape(bs, n_vars, -1).reshape(bs, n_vars, num_patch, patch_len).permute(0, 2, 1, 3)
...
class PretrainHead(nn.Module):
    ...
    def forward(self, x, patch_size):
        ...
        """
        Linear = nn.Linear(self.d_model, patch_size)
        Linear.weight.data = resample_patchemb(old=self.linear.weight.data.T, new_patch_len=patch_size).T
        Linear.bias.data = resample_patchemb(old=self.linear.bias.data.unsqueeze(0), new_patch_len=patch_size).squeeze(0)
