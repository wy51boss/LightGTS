...
class st_decompose(nn.Module):
    def __init__(self, kernel_size):
        super(st_decompose, self).__init__()
        self.seasonality_model = FourierLayer(k=3)
        self.trend_model = series_decomp(kernel_size)

    def forward(self, x):
        ###添加时序分解模块#####
        res, trend = self.trend_model(x)
        season, _ = self.seasonality_model(res)
        resid = res - season
        return  trend, season, resid

if __name__ == "__main__" :
    bs, t, c = 64, 1000, 7
    x = torch.randn((bs, t, c))
    decompose = st_decompose(kernel_size=3)
    trend, season, res = decompose(x)
    plt.plot(x[:,1])
    plt.plot(trend[:,1])
    plt.plot(season[:,1])
    plt.show()
